...
void CharPattern_PrintResult(const CharPattern_PredictedInfo* resultInfo) {
	...
	sprintf((char*) message, "Character: %c", (char) resultInfo->predicted_char);
	BSP_LCD_DisplayStringAtLine(start_line++, message);
	sprintf((char*) message, "Time taken: %lu ms", resultInfo->duration);
	BSP_LCD_DisplayStringAtLine(start_line++, message);
	sprintf((char*) message, "Accuracy: %.5f", resultInfo->confidence);
	BSP_LCD_DisplayStringAtLine(start_line++, message);
	sprintf((char*) message, "Draw Next Character:");
	BSP_LCD_DisplayStringAtLine(start_line++, message);
	start_line++;
	start_line++;
	start_line++;
	...
	start_line++;
	start_line++;
	sprintf((char*) message, "Press Reset.");
	BSP_LCD_DisplayStringAtLine(start_line++, message);
	sprintf((char*) message, "To Restart");
	BSP_LCD_DisplayStringAtLine(start_line++, message);
}
